...
from src.metadata import data_cleanup
...
def __add_missing_classes(train_data_path, test_data_path):
    ...
    train_classes = sort(list(df_train[classification_col].unique()))

    df_test = df_get(test_data_path, delimiter=',')
    test_classes = sort(list(df_test[classification_col].unique()))
...
def __copy_random_record_of_class(from_df, from_file_path, to_df, to_file_path, classes=None):
    ...
    cnt = 0
    for clz in classes:
        sample_df = from_df[from_df[classification_col] == clz].sample(1)
