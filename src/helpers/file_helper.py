...
def is_not_header(line, header_line):
    return not line[0] == header_line[0]
...
def delete_dir_content(output_directory):
    files = glob(output_directory + '/*')
    for f in files:
        os.remove(f)

    logging.info('Content of ' + output_directory + ' is deleted.')
